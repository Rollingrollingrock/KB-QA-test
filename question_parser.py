...
class QuestionParser():
    ...
    def parser_main(self, res_classify):
        ...
        for question_type in question_types:
            each_sql = {}
            each_sql['question_type'] = question_type
            sql = []
    ...
    def sql_transfer(self, question_type, entities):
        ...
        if question_type == 'stockid_conceptget':
            sql = [
                "MATCH (m:{0})-[r:ConceptInvolved]->(n:Concept) where m.stock_code = '{1}' return m.stock_id, m.sotck_name, r.name, n.name".format(
                    self.pDate, i) for i in entities]

        # 按股票名称查询所属概念
        elif question_type == 'stockname_conceptget':
            sql = [
                "MATCH (m:{0})-[r:ConceptInvolved]->(n:Concept) where m.stock_name = '{1}' return m.stock_id, m.stock_name, r.name, n.name".format(
                    self.pDate, i) for i in entities]
        ...
        # 根据概念查股
            ...
            for i in range(length):
                part1_list.append(
                    "(m:{0})-[r#i#:ConceptInvolved]->(n#i#:Concept)".format(self.pDate).replace('#i#', str(i)))
            ...
            sql = ["MATCH #part1# where #part2# return m.stock_id, m.stock_name, #part3#".replace('#part1#',
                                                                                                  part1).replace(
                '#part2#', part2).replace('#part3#', part3)]
        ...
        # 按股票代码查询实际控制人
            ...
            sql = [
                "MATCH (m:{0})-[r:IsControlledBy]->(n:Controller) where m.stock_id = '{1}' return m.stock_id, m.sotck_name, r.name, n.name, n.type".format(
                    self.pDate, i) for i in entities]

        # 按股票名称查询实际控制人
        elif question_type == 'stockname_controllerget':
            sql = [
                "MATCH (m:{0})-[r:IsControlledBy]->(n:Controller) where m.stock_name = '{1}' return m.stock_id, m.stock_name, r.name, n.name, n.type".format(
                    self.pDate, i) for i in entities]

        # 根据实际控制人查股
        elif question_type == 'controller_stockget':
            sql = [
                "MATCH (m:{0})-[r:IsControlledBy]->(n:Controller) where n.name = '{1}' return m.stock_id, m.stock_name, n.name".format(
                    self.pDate, i) for i in entities]

        # 按股票代码查询所属行业
        elif question_type == 'stockid_industryget':
            sql = ['MATCH (m:Company{stock_code:{0}})-[r:IndustryInvolved]->(x) RETURN x.Tertiary_industry, x.Secondary_industry, x.Primary_industry']
                #"MATCH (m:{0})-[r:IndustryInvolved]->(n:Industry) where m.stock_id = '{1}' return m.stock_id, m.stock_name, r.name, n.name".format(i) for i in entities]

        # 按股票名称查询所属行业
        elif question_type == 'stockname_industryget':
            sql = [
                "MATCH (m:{0})-[r:IndustryInvolved]->(n:Industry) where m.stock_id = '{1}' return m.stock_id, m.stock_name, r.name, n.name".format(
                    self.pDate, i) for i in entities]

        # 按行业查询股票
        elif question_type == 'industry_stockget':
            sql = [
                "MATCH (m:{0})-[r:IndustryInvolved]->(n:Industry) where n.name = '{1}' return m.stock_name, n.name".format(
                    self.pDate, i) for i in entities]
        ...
        # 查市场类型
            ...
            for i in range(length):
                part1_list.append(
                    "(m:{0})-[r#i#:MarketTypeIs]->(n#i#:MarketType)".format(self.pDate).replace('#i#', str(i)))
                part2_list.append("n#i#.name = '{0}'".format(entities[i]).replace('#i#', str(i)))
                part3_list.append("n#i#.name".replace('#i#', str(i)))
            part1 = ",".join(part1_list)
            part2 = " and ".join(part2_list)
